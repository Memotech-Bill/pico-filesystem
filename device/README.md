...
can be opened, read from or written to using `stdio.h` routines.
...
* `output` = Name of the routine to output characters written to
  the device, one character at a time. Set this to NULL for an input
  only device.

* `ndata` = Size of the buffer to store received characters until
  they are read. This must be a power of 2.

* `mode = Value which determines when a read from the device will return:
  * `IOC_MD_FULL`   - Only return when the buffer is full
  * `IOC_MD_NBLOCK` - Never block waiting for input
  * `IOC_MD_ANY`    - Return as soon as at least one character input
  * `IOC_MD_CHR`    - Return when specified character received
  * `IOC_MD_CR`     - Return when Carriage Return character received
  * `IOC_MD_LF`     - Return when Line Feed character received
  * `IOC_MD_TLF`    - Replace terminating character by Line Feed
  * `IOC_MD_ECHO`   - Echo input characters to output
...
In CMake specify the `pfs_dev_gio` link library for this device driver.

### UART driver

This driver provides support for either of the Pico hardware UARTs.
To create a `pfs_device` for one of the UARTs call:

```c
struct pfs_device *pfs_dev_uart_create (int uid, SERIAL_CONFIG *sc);
```

* `uid` - UART Instance (0 or 1).
...
In CMake specify the `pfs_dev_uart` link library for this device driver.

### USB Keyboard Driver

This uses the Pico in USB host mode, and returns characters typed
...
the parameter to `pfs_dev_kbd_fetch`. If a `keymap` is
...
table with format given in `pfs_dev_keymap.h`:
...
The value of `nkey` gives the highest scan code for which an
...
`EXTEND_ASCII` is set to a non-zero value, then these keys
...
It is not possible to define a key that will produce an ASCII
...
Bit 7 clear for key press events and bit 7 set for key release events.
...
Bit 7 of both bytes is clear for key press events and set on both bytes for release events.
...
* `IOC_MD_ANY`    - Return as soon as at least one character input
* `IOC_MD_CHR`    - Return when specified character received
* `IOC_MD_CR`     - Return when Carrage Return character received
* `IOC_MD_LF`     - Return when Line Feed character received
* `IOC_MD_TLF`    - Replace terminating character by Line Feed
* `IOC_MD_ECHO`   - Echo input characters to output
...
wait forever. To have zero timeout, set the mode `IOC_MD_NBLOCK`.
